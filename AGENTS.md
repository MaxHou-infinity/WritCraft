...
For strict structured model output, the prompt and scanner must state the same envelope and repeated-item schema: exact keys, first/last JSON characters, no fences/peripheral prose/comments/second object, and an explicit array contract for every repeated list field. Never strip, extract, or coerce a plausible value locally. A bounded retry may run only for an exact content-free envelope or structure classification, must share one retry budget across classifications, must not echo the rejected output, must revalidate every frozen dependency before the second provider call, and must never make a third call. Use repository-owned focused scripts such as `npm run verify:plan`; if none exists, confirm test entry paths from `package.json` or `rg --files tests` before invoking them. A remembered filename is not evidence.
