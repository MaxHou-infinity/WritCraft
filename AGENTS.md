...
Test-only process startup budgets must be separate from injected operation deadlines. A one-second helper bind budget can fail only under the full compile/spawn load while focused tests stay green; preserve the red run, keep the production timeout unchanged, raise only the named test startup/recovery budget, and retain the short post-readiness deadline that creates the intended crash or unknown outcome.
