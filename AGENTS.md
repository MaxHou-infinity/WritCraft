...
Watcher-driven E2E must prove quiescence before minting fresh authority. A drained promise queue or one observed generation advance is insufficient when native events, debounce, and polling fallback can arrive in separate waves. Wait for the queue to drain and the authoritative generation to remain stable for a window covering the fallback interval; on timeout, capture bounded UI/state diagnostics before any retry.
