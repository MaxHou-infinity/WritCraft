...
Writing Navigation checkpoint 0.0BZ is Main-only foundation: strict read-only generation, cache, watcher barrier, cancellation, and proposal IPC are signed off. Renderer, action handoff, structure-skeleton transaction, old public Plan removal, and real-Electron acceptance remain open. Do not call the feature user-available or use the old Plan UI as its implementation shortcut.
