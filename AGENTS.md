...
Writing Navigation checkpoint 0.0CA has signed off the Main-only read path and open/Research/Changes handoff. Renderer, structure-skeleton transaction, old public Plan removal, and real-Electron acceptance remain open. Every action run/cancel must bind the same opaque attempt ID; a late cancel/finally from attempt A must never affect retry B. Retryable failure, timeout, or cancellation preserves a still-current action, while success, stale, or replay terminates it. Do not call the feature user-available or use the old Plan UI as its implementation shortcut.
...
For strict structured model output, repeated real failures after prompt tightening mean the generation protocol is wrong, not that the prompt needs another adjective. Prefer a provider-supported structured channel with one complete schema over free-form large JSON. For Plan, production authority is exactly one named `submit_project_plan` tool input; thinking/text is non-authoritative, and legacy text JSON must not be revived. Count every raw `tool_use` block before validating the matching one, otherwise one valid plus one malformed call can bypass uniqueness. Keep Main exact-key, array, path, dependency, revision and resource validation; never strip, extract or coerce a plausible value locally. A bounded structure retry must not echo rejected input, must revalidate every frozen dependency before the second call, and must never make a third call. Synthetic tests must inject failures in the last allowed repeated item, not only the first or second task. Provider support inferred from compatibility still needs one real canary; automation cannot certify undocumented named-choice behavior. Use repository-owned focused scripts such as `npm run verify:plan`; if none exists, confirm test entry paths from `package.json` or `rg --files tests` before invoking them.
