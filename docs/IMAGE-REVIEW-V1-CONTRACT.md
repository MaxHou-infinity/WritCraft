...
> Status: **0.1.2 frozen compatibility contract; corrected source passed affected-path author acceptance and shipped to npm `preview` / GitHub prerelease on 2026-08-03**. The original author run exposed prompt leakage and stale settlement as retained red evidence; the published behavior inserts neutral `章节配图` alt text, permits same-project forward-generation keep, and blocks unsafe delete after manuscript change. Editable alt/caption and retained-asset cleanup remain later-version P2 candidates, not open 0.1.2 gates.
