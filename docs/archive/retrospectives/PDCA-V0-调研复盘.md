...
> **2026-07-28 0.0Y 后产品能力审计复盘（历史实施前审计）**：版本号与测试数字没有漂移，但“主链签字”被部分文档误写成“完整能力已实现”。源码对照确认三项真实缺口：Chat 只发送本轮问题，没有 Main-owned 最近对话摘要；超长 `edit.md` 会整体 fail closed，没有章节化保留与 Inspector 披露；普通 Markdown 虽有 Main 回收/恢复 service，却没有 preload/Main IPC 和 App 恢复入口。正确做法是把大模块拆成可验证能力逐项标记，不用一个 ✅ 覆盖未接 UI 或未实现的 PRD 子条目。其中 `edit.md` 分区项已由下方 0.0Z 收口，Chat 多轮项已由下方 0.0AA 收口；当前只剩普通 Markdown 回收 UI，顺序只看最新状态文件。
...
> **2026-07-28 0.0AA Main-owned Chat 多轮复盘**：把最近对话放到 Main 后，Renderer 只隐藏旧回答并不等于旧请求已失去写入会话的权力；新请求必须在任何 save/flush/context 等待之前先取消 Main pending，“新对话”和同项目重开也必须同时清理 UI 与 Main 会话。复审还推动失败重开保持零会话副作用：先成功读取 tree/`edit.md`，再失效旧会话，不能让失败操作破坏仍在使用的上下文。集成红灯同样暴露两类测试误差：fixture 不存在的 `instanceId` 不能代替运行时权威值；外部恢复 `edit.md` 后必须等待生产 flush barrier，不能靠时间猜 watcher 已结束。最终 Conversation **8/8**、Chat Context **12/12**、Resolver **26/26**、Inspector **9/9**、Policy **10/10**、Network **15/15**、Workspace **10/10**、完整 test/verify、Persistent **3/3**、package **8/8**、release **7/7**、最终源码真实 Electron 连续两次 **34/34**，独立复审 P0/P1/P2=0。可复用教训：跨进程状态必须在最早的权威边界撤销；一次用户动作涉及的可见状态与私有状态必须作为同一事件；失败操作不得提前提交清理副作用；红运行应分类为产品、环境或测试权威问题并永久保留。

> **2026-07-30 0.0AU 真实作者误撤销复盘**：History 的存储顺序和撤销事务都正确，但“每张卡只显示文件数量、每个按钮都叫安全撤销、确认框不说目标”使作者误撤销了较早的 `edit.md`，最新章节记录反而仍为 applied。根因不是后端数据错误，而是破坏性操作缺少目标辨识与风险分层。修复必须同时覆盖卡片、按钮、确认框和真实 Electron：单文件显示精确路径，最新记录显式标记，旧记录警告“不是最新”，`edit.md` 明示会改变项目 Prompt 与后续 AI 上下文。旧副本不自动修补，而作为红灯证据失效；生产源码变化后形成新 candidate 和 fresh copy 重跑受影响旅程。可复用教训：事务安全只证明“系统准确执行了点击”，不证明“用户点的是自己理解的对象”；任何撤销/删除/覆盖操作都必须在最终确认点重述精确对象与非默认风险。
...
以下是 2026-07-14 调研阶段的启动假设，已被实际选型和后续里程碑覆盖：编辑器采用原生 `contenteditable`，正式 PRD 已存在，Plan Mode 可跳过。0.0Z 已签字 `edit.md` section-aware 上下文编译与 Inspector 章节披露，0.0AA 已签字 Main-owned Chat 最近对话与多轮连续性；当前 Next Action 只看 `v0/DEVELOPMENT-STATUS.md`：只处理普通 Markdown 回收区可见列表与单项恢复 UI。`docs/AUTHOR-ACCEPTANCE-V1-CONTRACT.md` 单独约束真实付费 API、真实作者旅程与发布验收。
