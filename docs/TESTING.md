...
| `103/103 passed` | **先别信。** 223 个脚本里有 **136 个**打印的是同一标识符的 `${x}/${x}`——分母就是分子，永远相等；另有 **14 个**打印硬编码的 `N/N` 字面量（实测，2026-09-12） |
...
- **超时保护只覆盖一部分**（2026-09-12 实测）：`v0/tests/` 下 **133 处子进程调用中，117 处没有
  `timeout` 选项**（仅 16 处有，分布在 9 个文件），未保护的调用分布在 27 个测试文件里；
  门禁 runner 自身也没有超时；`npm test` 是一条 `&&` 链。**一处 hang 仍可能挂住整条链。**
  （审计 C 报告为"113 处全部无超时"，经父 agent 精确重测后更正为上述数字。）
