...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, Structure, Navigation generation/canonical-evidence opening/helpfulness, and Navigation→Changes are closed and must not be rerun. 0.0CK closed Research handoff retryability; 0.0CL replaced fragile model-authored Research text JSON with one named structured tool. The author has confirmed that the Research page receives the question and original evidence; only the actual “研究所选来源” run and downstream judgment remain on this shortest chain. Image and Graph/recovery remain open. None of this authorizes publication.
