...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, and the fresh empty-project Structure journey are closed; Structure is bound to production commit `2f9f714` and created six reviewed skeleton files with no recovery residue or History. The retired Plan accumulated nine real, zero-write failures; the latest operation `db1c6915ed38430aa827e441666b8218` failed after 22,312 ms with before/after 0. These runs prove the write guard, but repeated schema repair did not prove author value. No further old-Plan canary is required. Existing-manuscript Navigation, Research, the authorized image journey, and Graph/recovery remain open. None of this authorizes publication.
