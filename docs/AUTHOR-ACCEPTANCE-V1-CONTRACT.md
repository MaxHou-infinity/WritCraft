...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. Project Card, Inline, and Chapter are closed. The seventh-copy Plan had three peripheral-text failures followed by two second-task `targetPaths` array-shape failures; all five attempts produced no plan, task, manuscript write, or History. Exact production candidate `94e6099` keeps strict parsing and zero local repair, explicitly requires every task list field to be a JSON array, and shares one same-operation retry budget across exact peripheral-envelope and array-shape classifications. The retry never carries rejected output, revalidates the tree plus frozen prompt/context before its second provider call, and never makes a third call. Plan 22/22, UI 16/16, handoff 15/15 + 5/5, Assistant 11/11, full test, approved-GUI verify, and forced real Electron 37/37 pass. One real same-goal rerun and Plan task handoff remain required; the author must not repeat Project Card, Inline, or Chapter. Research, the authorized image journey, and Graph/recovery also remain open. None of this constitutes complete V0 acceptance or publication authorization; current totals remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
