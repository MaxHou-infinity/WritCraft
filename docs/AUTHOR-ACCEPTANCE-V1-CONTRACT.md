...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, the fresh empty-project Structure journey, and existing-manuscript Navigation generation/canonical-evidence opening are closed. Structure remains bound to `2f9f714`; Navigation is bound to evidenceRef fix `d28c440` and fresh copy 9. Copy 8 remains red evidence only. In copy 9 the author obtained three suggestions with Main-owned evidence, opened the bound chapter, and explicitly judged the suggestions helpful; two content-free success events were 18,314 ms and 13,562 ms with before/after 0, post-manifest History=0, recovery=0, and public files equal to the unchanged source. Current-card Changes/Research, the authorized image journey, and Graph/recovery remain open. No further old-Plan, Structure, or Navigation-generation canary is required. None of this authorizes publication.
