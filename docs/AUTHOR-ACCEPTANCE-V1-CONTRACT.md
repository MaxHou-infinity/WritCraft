...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. The 0.1.1 candidate closed full test/verify, Persistent **3/3**, npm Preview **10/10**, local/public installed **2/2**, npm10/arm64 and npm11/x64 **2/2** each, production audit at zero vulnerabilities, and two consecutive real Electron **37/37** runs after retained red timing evidence. On 2026-07-28 the configured Coding Plan credential also passed one gated synthetic `image-01` call: JPEG, 1280×720, 269,177 bytes, 19,333 ms, zero manuscript insertion. On 2026-07-30 the owner authorized and explicitly confirmed a path-only organizing candidate as the acceptance source, selected the formal-copy parent, and authorized one real `image-01` journey capped at 20 CNY. The first formal copy and candidate `45b1815` are retained only as red diagnostic evidence after the author undid an older `edit.md` record rather than the newest chapter record. Candidate `9a05c44` added exact targets and warnings; a second fresh copy then proved project-card, Inline reject/accept and exact Safe Undo disk/History truth, but exposed a false AI timer. `89658f0` fixed terminal cleanup, and `6ed6f89` added truthful `NO_KEY` recovery plus exact-project draft preservation. A new independent review of `6ed6f89` nevertheless found P0=0/P1=2/P2=1: Safe Undo reverted to “AI 正在处理” after ten seconds, its late `finally` could clear a newer project task, and NO_KEY lacked dynamic Workspace evidence. Exact production candidate `30a06b0` fixes those owner/timing boundaries and adds Renderer dynamic 29/29 plus Onboarding Workspace dynamic 4/4; full test/approved verify, real Electron 37/37, npm Preview 10/10, local and both official npm10/arm64 and npm11/x64 installed matrices 2/2, and production audit at zero vulnerabilities pass. A second independent review reports P0=0/P1=0/P2=1; docs-only commit `b6041b6` closes that wording P2 without changing production. The third fresh isolated copy was created from the unchanged accepted source with 20 files / 608,433 bytes, `sourceUnchanged=true`, and source digest `9d0898863636da74615481a669c3e40362a60176249870e6d1793380c85b4380`; its manifest `createdAt` is `2026-07-30T10:56:11.255Z`. Its first author-operated project-card attempt ended safely with one `generated → discarded` operation, no new History, no `edit.md` hash change, and no initial files. This proves the no-write path but not the required `edit.md` ChangeSet acceptance and disk landing, so the project-card journey remains open on the same valid candidate/copy. Chapter/Plan, Research, paid `image-01`, and Graph/restart recovery have not begun on this valid candidate. Because the source already contained private `.writcraft` history, only events and History entries after each copy manifest `createdAt` count as that run. None of the release, synthetic, preparation, partial journey, failed-profile events, or invalidated-copy evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
