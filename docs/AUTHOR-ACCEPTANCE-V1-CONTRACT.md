...
> Current execution checkpoint (2026-08-02): author copy 11 is retained as red evidence only. Its first Navigation attempt was correctly rejected before provider work because `edit.md` was active and no manuscript context was selected, but the UI exposed a generic AI failure. The source and all 14 Markdown files remained unchanged and no post-manifest History entry was created. Any production fix invalidates this copy for final sign-off; create a fresh numbered copy from the unchanged selected source after the new exact candidate passes verification and review.
