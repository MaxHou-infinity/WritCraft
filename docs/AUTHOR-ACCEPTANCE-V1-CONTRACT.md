...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. The 0.1.1 candidate closed full test/verify, Persistent **3/3**, npm Preview **10/10**, local/public installed **2/2**, npm10/arm64 and npm11/x64 **2/2** each, production audit at zero vulnerabilities, and two consecutive real Electron **37/37** runs after retained red timing evidence. On 2026-07-30 the owner authorized and explicitly confirmed the acceptance source, formal-copy parent, and one real `image-01` journey capped at 20 CNY. Old candidates and copies remain diagnostic only. Candidate `30a06b0` closed Safe Undo ownership and NO_KEY dynamic evidence, then the third fresh copy completed a real project-card `generated → accepted` application but exposed a P1 terminal-copy contradiction; that candidate and journey are invalid for final sign-off. Exact candidate `5ed4147` fixes the committed terminal truth, passes full approved-GUI verify, forced real Electron 37/37, npm Preview 10/10, all three installed checks at 2/2, a zero-vulnerability production audit, and independent review P0=0/P1=0. The fourth fresh copy was created from the unchanged eligible source at `2026-07-30T13:12:06.288Z`. Its real-author project card now passes: operation `32b817da…` is `generated → accepted`, one applied `edit.md` History record contains two accepted and zero rejected hunks, disk and History advance from `e9e9f7c…` to `8b68a5b…`, the original source remains unchanged, and the live UI truthfully confirms the accepted edit was retained while initial files were skipped. The project-card subjourney is closed and must not be repeated; remaining journeys start with an independent Inline Rewrite rejection. None of the release, synthetic, preparation, failed-profile, or invalidated-copy evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
