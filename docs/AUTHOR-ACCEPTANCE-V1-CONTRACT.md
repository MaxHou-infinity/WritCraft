...
> Status: **frozen contract; 0.0V watcher ancestor traversal is technically signed, while real-author and paid acceptance remain open**. Current evidence is author preflight **48/48**, real-API offline **15/15** (**63/63, 0 network**), native worker **10/10**, Watcher **30/30**, package **8/8**, local release **7/7**, full test/verify, Persistent **3/3**, forced Electron **32/32**, and independent review **P0=0/P1=0/P2=2**. The configured credential is Coding Plan and the recent project remains ineligible; real-author and paid gates remain closed.
> Boundary update: 0.0V uses a separately attested native helper to traverse below a bound project-root fd and binds both native helpers through the App and standard-unzip ZIP. It does not close the initial root-open outer-ancestor race, add an independent serialized-request metadata byte cap, or remove the 0.0U same-UID 0700 empty-directory residual.
