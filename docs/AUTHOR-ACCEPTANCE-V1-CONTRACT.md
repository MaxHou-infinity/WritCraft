...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. The 0.1.1 candidate closed full test/verify, Persistent **3/3**, npm Preview **10/10**, local/public installed **2/2**, npm10/arm64 and npm11/x64 **2/2** each, production audit at zero vulnerabilities, and two consecutive real Electron **37/37** runs after retained red timing evidence. On 2026-07-28 the configured Coding Plan credential also passed one gated synthetic `image-01` call: JPEG, 1280×720, 269,177 bytes, 19,333 ms, zero manuscript insertion. On 2026-07-30 the owner authorized and explicitly confirmed a path-only organizing candidate as the acceptance source, selected the formal-copy parent, and authorized one real `image-01` journey capped at 20 CNY. The first formal copy proved project-card and Inline reject/accept mechanics, but its Safe Undo attempt exposed a P1 anti-misoperation failure: the author undid an older `edit.md` record while the newest chapter record remained applied. That copy and candidate `45b1815` are therefore retained only as red diagnostic evidence. Candidate `9a05c44` added exact targets, latest-record marking, and separate non-latest/Project-Prompt warnings. A second fresh isolated copy then completed project-card, Inline reject, Inline accept, and exact Safe Undo: the latest chapter History entry is `undone`, the chapter equals its before revision, and `edit.md` is unchanged. That successful mutation nevertheless exposed another P1: the completed undo kept an AI generation timer alive and falsely displayed “generating cross-file changes” for at least 159 seconds, although no provider request, ChangeSet, History, or Markdown write occurred. Intermediate candidate `89658f0` added undo-specific non-AI progress and terminal cleanup. Exact candidate `6ed6f89` additionally maps stable `NO_KEY` to a truthful pre-provider message and existing Settings action while preserving the exact-project draft across the detour; success and project switch clear the draft. Focused Onboarding UI is 12/12, dynamic Renderer 27/27, Main/Preload 14/14, Workspace 22/22, full test/verify pass, real Electron is 37/37, npm Preview is 10/10, local and both official npm10/arm64 and npm11/x64 installed matrices are 2/2 each, and production audit is zero vulnerabilities. Because these production changes invalidate `9a05c44`, `89658f0`, and their affected final-journey evidence, the second copy is retained as functional/red diagnostic evidence only. Independent review of `6ed6f89` is the remaining technical gate before a third fresh isolated copy restarts all five journeys. Chapter/Plan, Research, paid `image-01`, and Graph/restart recovery have not begun on a valid final candidate. Because the source already contained private `.writcraft` history, only events and History entries after each copy manifest `createdAt` count as that run. None of the release, synthetic, preparation, partial journey, failed-profile events, or invalidated-copy evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
