...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, Structure, and Navigation generation/canonical-evidence opening/helpfulness are closed. Copy 9 produced two zero-write Changes red runs: `MODEL_OUTPUT_TOO_LARGE`, then `PATCH_NEW_TEXT_TOO_LARGE`. 0.0CI replaces model-chosen anchors with Main-owned revision-bound ranges, a 7 KiB/8192-token envelope and at most one internal structure correction; automated verification and independent review are complete, but one author retry on the latest source is still required before Changes can close. Research, image, and Graph/recovery remain open. No old-Plan, Structure, or Navigation-helpfulness rerun is required. None of this authorizes publication.
