...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. Project Card, Inline, and Chapter are closed. The seventh-copy Plan now has six zero-write failures: three peripheral-text, two milestone-1/task-2 array-shape, and operation `ef44ed234ceb417d9d8723fa918e3173` at milestone 7/task 1 after 107,623 ms. That last result invalidates `94e6099` as a closed fix. 0.0BU moves Plan authority from free-form text JSON to exactly one `submit_project_plan` tool input with a complete schema and named tool choice; Main still enforces exact keys, arrays, paths, dependencies, revisions and one bounded structure retry without local coercion or text fallback. Focused, full, approved-GUI and forced Electron automation pass; two timing red runs outside the changed Plan boundary were preserved and each failed to reproduce on one same-source confirmation. Independent review is P0=0/P1=0/P2=0, while one real MiniMax named-tool canary remains required. The author must not repeat Project Card, Inline, or Chapter, and must not repeatedly click Plan retry before the new candidate is restarted. Research, the authorized image journey, and Graph/recovery also remain open. None of this constitutes complete V0 acceptance or publication authorization; current totals remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
