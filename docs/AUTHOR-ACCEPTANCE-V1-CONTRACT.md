...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. The 0.1.1 candidate closed full test/verify, Persistent **3/3**, npm Preview **10/10**, local/public installed **2/2**, npm10/arm64 and npm11/x64 **2/2** each, production audit at zero vulnerabilities, and two consecutive real Electron **37/37** runs after retained red timing evidence. On 2026-07-30 the owner authorized and explicitly confirmed the acceptance source, formal-copy parent, and one real `image-01` journey capped at 20 CNY. Old candidates and copies remain diagnostic only. Candidate `30a06b0` closed Safe Undo ownership and NO_KEY dynamic evidence, then the third fresh copy completed a real project-card `generated → accepted` application: one applied `edit.md` History record, two accepted hunks, zero rejected hunks, and source snapshot unchanged. The same journey exposed a P1 terminal-copy contradiction: after the committed edit, “skip initial files” claimed the `edit.md` result had not changed. Production Renderer now branches on committed-edit truth and dynamic evidence is 30/30. Because production code changed after the journey, `30a06b0` and the third-copy project-card evidence are invalid for final sign-off. Two full-test runs also retained native helper bind-timeout reds caused by a test-only one-second bind/ordinary-recovery budget; production remained at 30 seconds, fault injection remains 100 ms after readiness, and the named test bind/ordinary-recovery budget is now five seconds. Focused native evidence is 20/20 and the subsequent full `npm test` passes. Exact candidate `5ed4147` has completed full approved-GUI verify, forced real Electron 37/37, npm Preview 10/10, local plus both official runtime installed matrices at 2/2, and a zero-vulnerability production audit. Independent review reports P0=0/P1=0/P2=1; its sole documentation-precision P2 is closed without changing production. The fourth fresh isolated copy was created from the unchanged eligible source at `2026-07-30T13:12:06.288Z`, and the App now opens that copy on `edit.md`; all five author journeys restart from this boundary. None of the release, synthetic, preparation, partial journey, failed-profile events, or invalidated-copy evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
