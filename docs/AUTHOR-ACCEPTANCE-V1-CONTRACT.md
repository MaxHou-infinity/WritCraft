...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. Project card, Inline, and Chapter are closed. The seventh-copy Plan then failed three times because the provider wrapped JSON in peripheral text; all three attempts produced no plan, task, manuscript write, or History. Exact candidate `0e7ad38` keeps strict parsing unchanged, makes the no-fence/no-peripheral contract explicit, and allows only one same-operation format retry for that exact classification. The retry never carries rejected output and revalidates the tree plus frozen prompt/context before its second provider call; a second invalid result terminates. Plan 21/21, UI 16/16, full test, approved-GUI verify, and forced real Electron 37/37 pass. One real same-goal rerun and Plan task handoff remain required; the author must not repeat Project Card, Inline, or Chapter. Research, the authorized image journey, and Graph/recovery also remain open. None of this constitutes complete V0 acceptance or publication authorization; current totals remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
