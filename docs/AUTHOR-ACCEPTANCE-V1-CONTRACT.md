...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, Structure, Navigation generation/canonical-evidence opening/helpfulness, and Navigation→Changes are closed and must not be rerun. 0.0CJ now restores same-App refresh/reopen from revalidated Main authority and fixes Research/Changes as simultaneous product affordances rather than a model choice. Research resumes only after final independent review and the shortest author refresh/reopen check; no prior helpfulness or Changes journey is repeated. Image and Graph/recovery remain open. None of this authorizes publication.
