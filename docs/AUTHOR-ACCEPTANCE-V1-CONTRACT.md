...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, Structure, Navigation generation/canonical-evidence opening/helpfulness, and Navigation→Changes are closed. After two zero-write red runs, `da5e8d6` produced three real review items; the author accepted and committed all three to one chapter, History recorded the Navigation provenance, and Safe Undo restored the exact source SHA with recovery empty and every public file equal to the authorized source. Do not rerun these journeys. Research, image, and Graph/recovery remain open. None of this authorizes publication.
