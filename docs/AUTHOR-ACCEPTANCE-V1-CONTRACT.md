...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, Structure, and Navigation generation/canonical-evidence opening/helpfulness are closed. Copy 9 produced two zero-write Changes red runs before 0.0CI replaced model-chosen anchors with Main-owned revision-bound ranges and a 7 KiB/8192-token envelope. The latest `da5e8d6` author retry produced three reviewable Changes items while the target SHA, recovery and History remained unchanged; real Diff generation is closed, while author decision, commit and Safe Undo remain open. Research, image, and Graph/recovery remain open. No old-Plan, Structure, or Navigation-helpfulness rerun is required. None of this authorizes publication.
