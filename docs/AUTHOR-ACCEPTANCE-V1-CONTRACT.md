...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, the fresh empty-project Structure journey, and existing-manuscript Navigation generation/canonical-evidence opening/helpfulness are closed. Structure remains bound to `2f9f714`; Navigation is bound to evidenceRef fix `d28c440` and fresh copy 9. Copy 8 remains red evidence only. Copy 9's first current-card Changes attempt reached the provider but failed closed with `MODEL_OUTPUT_TOO_LARGE`; no Diff, post-manifest History, recovery, or public-file change was created. 0.0CH replaces that free-text handoff with a bounded named localized-edit tool and has passed full automated and real-Electron verification, but still requires one author retry on the latest source before Changes can close. Current-card Research, the authorized image journey, and Graph/recovery remain open. No further old-Plan, Structure, or Navigation-generation canary is required. None of this authorizes publication.
...
3. **Structure and navigation:** in a fresh empty project, compare 2–3 structure alternatives, edit one chapter skeleton, and explicitly confirm all-or-nothing skeleton-file creation. In the existing author copy, inspect 1–3 evidence-backed next-step suggestions, open one target, and route one suggestion to Changes for reject or accept plus Safe Undo. The retired Plan task graph is not rerun.
4. **Research:** inspect Claim / Source / Boundary, explicitly rate evidence as matching or not matching, then accept or reject a Research→Changes proposal. Exact-quote validation proves location only, not factual truth.
