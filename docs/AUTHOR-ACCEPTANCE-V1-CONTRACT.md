...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. On 2026-07-30 the owner authorized and confirmed the acceptance source, formal-copy parent, and one real `image-01` journey capped at 20 CNY. The fourth fresh copy completed the real project-card subjourney; it is closed and must not be repeated. Its subsequent Inline acceptance exposed that old candidate `5ed4147` skipped the required author instruction after `⌘K`, so that affected Inline evidence is diagnostic only. Exact production candidate `9b21a9d` implements request v2 and the selection-adjacent command composer, passes full test/approved-GUI verify, forced real Electron 37/37, and third-round independent review P0=0/P1=0/P2=0. In the fifth fresh eligible copy, the author completed an accepted Inline whose applied History and disk advanced the first chapter from `1bdb3a5c…` to `f9b89aac…`, then an independent rejected Inline with zero extra writes, and finally Safe Undo on the exact latest chapter record. History is now `undone`; the target, all 14 Markdown files, byte count and combined digest exactly match the pre-operation baseline, the UI confirms “原文已恢复” and “安全撤销已结束”, and the unchanged source remains `9d089886…`. The affected Inline rerun is closed; current author work proceeds to Chapter/Plan, then Research, image, and Graph/recovery. None of the release, synthetic, preparation, failed-profile, or invalidated-copy evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
