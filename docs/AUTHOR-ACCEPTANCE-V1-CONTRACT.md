...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. The 0.1.1 candidate closed full test/verify, Persistent **3/3**, npm Preview **10/10**, local/public installed **2/2**, npm10/arm64 and npm11/x64 **2/2** each, production audit at zero vulnerabilities, and two consecutive real Electron **37/37** runs after retained red timing evidence. On 2026-07-28 the configured Coding Plan credential also passed one gated synthetic `image-01` call: JPEG, 1280×720, 269,177 bytes, 19,333 ms, zero manuscript insertion. On 2026-07-30 the owner authorized a path-only organizing candidate, a formal-copy parent, and one real `image-01` journey capped at 20 CNY. The candidate preserves all 20 source files byte-for-byte while mapping the existing manuscript and source material into `chapters/` and `references/`; preflight is now eligible with 12 chapter files, 4,748 visible Chinese characters, and one source file. The original digest remains unchanged. No formal acceptance copy or paid call may occur until the author explicitly confirms this candidate as the new acceptance source. The earlier ineligible-structure and “no `edit.md`” results are historical and must not be reused as current fact. None of the release, synthetic, or preparation evidence constitutes author quality, adoption, cost, or complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
