...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. The 0.1.1 candidate closed full test/verify, Persistent **3/3**, npm Preview **10/10**, local/public installed **2/2**, npm10/arm64 and npm11/x64 **2/2** each, production audit at zero vulnerabilities, and two consecutive real Electron **37/37** runs after retained red timing evidence. On 2026-07-28 the configured Coding Plan credential also passed one gated synthetic `image-01` call: JPEG, 1280×720, 269,177 bytes, 19,333 ms, zero manuscript insertion. On 2026-07-30 the owner authorized and then explicitly confirmed a path-only organizing candidate as the new acceptance source, selected the formal-copy parent, and authorized one real `image-01` journey capped at 20 CNY. The candidate preserves all 20 source files byte-for-byte while mapping the existing manuscript and source material into `chapters/` and `references/`; preflight is eligible with 12 chapter files, 4,748 visible Chinese characters, and one source file. The formal author-copy transaction then returned `copyCreated: true / sourceUnchanged: true`; its fresh copy remains eligible and adds only the private acceptance manifest. The source and original digests remain unchanged. The current App has opened this fresh copy at project-card step 1, but no journey or paid call has yet been completed. The earlier ineligible-structure and “no `edit.md`” results are historical and must not be reused as current fact. None of the release, synthetic, preparation, or launch evidence constitutes author quality, adoption, cost, or complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
