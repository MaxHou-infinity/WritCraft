...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. On 2026-07-30 the owner authorized and confirmed the acceptance source, formal-copy parent, and one real `image-01` journey capped at 20 CNY. The fourth-copy project card and fifth-copy Inline accept/reject/Safe Undo subjourneys are closed and must not be repeated. The fifth-copy Chapter attempt then failed at `b5.content` after about 41.4 seconds; no ChangeSet, History, or manuscript write was created, so this is diagnostic evidence rather than a passed journey. Exact candidate `5eb11bd` preserves strict JSON/schema/id failure, canonicalizes only CRLF/outer line breaks, and permits at most one content-boundary regeneration for the current block across the whole chapter after revalidating all frozen dependencies. It never echoes rejected model content or automatically writes. Chapter 21/21, full test, approved-GUI verify, and forced real Electron 37/37 pass. The candidate is pushed and the sixth fresh eligible copy is open in the source App; the author now reruns only Chapter/Plan. The unchanged source remains `9d089886…`; Research, image, and Graph/recovery remain open. None of the release, synthetic, preparation, failed-profile, or invalidated-copy evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
