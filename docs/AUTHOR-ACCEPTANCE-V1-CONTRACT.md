...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.1 / 0.1.2` target**. Project Card, Inline, Chapter, and the fresh empty-project Structure journey are closed; Structure is bound to production commit `2f9f714` and created six reviewed skeleton files with no recovery residue or History. The retired Plan accumulated nine real, zero-write failures and requires no further canary. Existing-manuscript Navigation then exposed two real `INVALID_MODEL_EVIDENCE` failures in copy 8; production commit `d28c440` replaced model-copied path/heading/quote with Main-owned request-local evidenceRefs and passed focused, full, real-Electron and independent P0/P1 gates. Copy 8 remains red evidence only. Fresh copy 9 was created from the unchanged authorized source by the production author-copy transaction and is the only current Navigation author journey. Research, the authorized image journey, and Graph/recovery remain open. None of this authorizes publication.
