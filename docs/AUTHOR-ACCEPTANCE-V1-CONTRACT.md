...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. The 0.1.1 candidate closed full test/verify, Persistent **3/3**, npm Preview **10/10**, local/public installed **2/2**, npm10/arm64 and npm11/x64 **2/2** each, production audit at zero vulnerabilities, and two consecutive real Electron **37/37** runs after retained red timing evidence. On 2026-07-28 the configured Coding Plan credential also passed one gated synthetic `image-01` call: JPEG, 1280×720, 269,177 bytes, 19,333 ms, zero manuscript insertion. On 2026-07-30 the owner authorized and explicitly confirmed a path-only organizing candidate as the acceptance source, selected the formal-copy parent, and authorized one real `image-01` journey capped at 20 CNY. The formal transaction returned `copyCreated: true / sourceUnchanged: true`, and the confirmed source digest remains unchanged. The owner has completed the project-card segment and both Inline decision directions. Rejection produced no Markdown write or new application/History. A later accepted operation created one target-only application/applied History record, and the target disk SHA-256 exactly matches its afterHash; its Safe Undo remains the immediate open gate. One earlier independent Inline generation had no decision event and remains reported separately rather than being silently counted as rejected. Because the source already contained private `.writcraft` history, only events and History entries after the copy manifest `createdAt` count as this run. Chapter/Plan, Research, the paid `image-01` decision, and Graph/restart recovery also remain open. None of the release, synthetic, preparation, or partial journey evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
