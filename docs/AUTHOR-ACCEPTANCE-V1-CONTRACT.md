...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. On 2026-07-30 the owner authorized and confirmed the acceptance source, formal-copy parent, and one real `image-01` journey capped at 20 CNY. The fourth fresh copy completed the real project-card subjourney: `edit.md` was explicitly accepted, its disk revision and applied History agree, initial files were skipped truthfully, and the unchanged source snapshot remained `9d089886…`; this subjourney is closed and must not be repeated. Its subsequent Inline acceptance exposed that old candidate `5ed4147` skipped the required author instruction after `⌘K`, so that affected Inline evidence is diagnostic only. Exact production candidate `9b21a9d` implements request v2 and the selection-adjacent command composer, passes full test/approved-GUI verify, forced real Electron 37/37, and third-round independent review P0=0/P1=0/P2=0. A fifth fresh eligible copy was created from the same unchanged source at `2026-07-30T14:48:31.566Z`; only Inline reject, independent accept, and exact-target Safe Undo must be rerun there before continuing Chapter/Plan, Research, image, and Graph/recovery. None of the release, synthetic, preparation, failed-profile, or invalidated-copy evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
