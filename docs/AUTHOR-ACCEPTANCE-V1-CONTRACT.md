...
> Status: **frozen contract; 0.0AC local release implementation has P0/P1 closed, while real-author acceptance remains open**. On 2026-07-28 the configured Coding Plan credential passed one gated synthetic `image-01` call: JPEG, 1280×720, 269,177 bytes, 19,333 ms, zero manuscript insertion. The author-selected project failed read-only eligibility with zero chapters, zero visible Chinese characters under the required structure, no `edit.md`, and no `references/`; no copy was created. Current-source `npm test`, Electron-enabled `npm run verify`, Persistent **3/3**, npm Preview **10/10**, installed public CLI/Renderer **2/2**, and two consecutive real Electron **35/35** runs passed after one retained 101 ms Graph performance red run. This does not constitute author quality/cost acceptance, npm10/x64 validation, or public registry publication.
...
- Only then rebuild from current source. For the initial npm route, follow `NPM-DEVELOPER-PREVIEW-V1-CONTRACT.md`; Developer ID/notarization/Gatekeeper are deferred unless a standalone macOS App is later distributed.
