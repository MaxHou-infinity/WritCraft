...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. Project Card, Inline, and Chapter are closed. The seventh-copy Plan now has seven zero-write failures. The latest, operation `706fa3e5529d4cda9cf0e2c3ecfe211c`, proves MiniMax admitted the named-tool request but returned `stop_reason=max_tokens` after 54,660 ms; no Plan, task card, manuscript or History write occurred. 0.0BW replaces the old roughly 419 KiB legal generation space with a 6,120-byte maximum Unicode fixture: one iterative plan may contain up to two milestones, two tasks per milestone and two targets per task with short fields and 80-character paths covering the current author project. Schema/Main reject controls and isolated surrogates; Schema/Main/Renderer count Unicode code points consistently and accept valid Emoji boundaries. It keeps the existing 8192-token/90-second bounds, refuses to auto-retry truncation, and retains the strict single `submit_project_plan` input plus all Main semantic/revision validation. The default model is already MiniMax-M3; the local Plan context gate remains eight explicit files/240 KiB until the China endpoint's M3/1M capacity is documented or safely proven. Focused/full tests, independent review and forced Electron 37/37 pass; one real provider canary remains required. The author must not repeat Project Card, Inline or Chapter, and must not click the stale error page's retry before the new candidate is restarted. Research, the authorized image journey, and Graph/recovery also remain open. None of this constitutes complete V0 acceptance or publication authorization; current totals remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
