...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. Project card, Inline, and Chapter are closed. Exact candidate `2073406` completed the real seventh-copy Chapter proposal and author acceptance; operation `2acf8b66…` produced one applied History on the first chapter, and disk exactly matches after revision `b0462641…` while the source remains unchanged. The live run exposed stale “selected / not yet written” hunk cards after commit; exact candidate `4bef659` replaces all pre-commit decisions with one trusted completion card. UX 9/9, Renderer dynamic 30/30, full test, approved-GUI verify, and forced real Electron 37/37 pass. Restarting the same copy on `4bef659` preserved the accepted text and after digest, kept the source at the before digest, and restored no pending review. The author must not repeat Chapter. Plan, Research, the authorized image journey, and Graph/recovery remain open. None of this constitutes complete V0 acceptance or publication authorization; current totals remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
