...
> Status: **frozen contract; 0.0T closed the separate Plan-write terminal-timing P2, while the 0.0S reserve/build-attestation boundary remains signed**. Current evidence (2026-07-28): author preflight **47/47**, real-API offline contract **15/15** (**62/62, 0 network**), package **8/8**, local release **7/7**, `npm test` and sandbox-external Electron-enabled `npm run verify` exit 0, Persistent Watcher **3/3**, Renderer dynamic **25/25**, and three full Electron runs **32/32** across implementation and independent review. The original Plan timeout is retained as red evidence; deterministic fault injection proved that terminal UI incorrectly waited for a duplicate post-reconciliation refresh, and the corrected batch passed independent review at **P0=0/P1=0/P2=0**. The configured credential is Coding Plan and the recent project fails the five-chapter/2,000-character/source requirements; real-author and paid gates remain closed.
> 0.0S boundary: Main owns an anonymous `0600 O_RDWR` reserve receipt on fd 5. The helper verifies it before any `mkdirat`, then writes and fsyncs the exact stage identity only after identity verification and parent fsync. Lost stdout/status can recover only from that receipt; missing dual evidence fails closed and preserves the unknown stage. Read-only receipt input is rejected pre-mutation; a known identity after parent-fsync/receipt-write failure is rechecked before cleanup, never adopted or deleted after replacement. Build signature is now part of the recipe and attestation binds build helper → App helper → ZIP helper/full App tree. Local ad-hoc App/ZIP were rebuilt from current source but remain non-distributable. The remaining local security P2s are only pure-Node ancestor traversal and the native `mkdirat→openat/fstat` micro-window.
