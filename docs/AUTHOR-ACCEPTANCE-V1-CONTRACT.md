...
> Status: **revised frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.2 / 0.1.2` target**. Project Card, Inline, Chapter, Structure, and Navigation generation/canonical-evidence opening/helpfulness are closed and must not be rerun. The author rejected the public Navigation → Research → Changes journey after excessive handoffs and a three-minute stale “处理中” state. Its replacement acceptance is the shortest unified-task chain defined by `UNIFIED-WRITING-TASK-V1-CONTRACT.md`. None of this authorizes publication.
...
> Current execution checkpoint (2026-08-02): author copy 12 confirmed Writing Navigation generation, then retained two `INVALID_MODEL_OUTPUT` unified-action failures as red evidence. Both failed before ChangeSet creation; 14/14 Markdown hashes still match the selected source and post-cutoff History is 0. Exact candidate `e8b9588` removes model-authored `oldText`, binds exactly one editable `rangeId` to each Main-owned same-file canonical suggestion, supplies only bounded adjacent read-only context, and maps a model-selected `editIntent` to a deterministic local action in Main. Navigation verification, complete tests, approved-GUI verification, forced real Electron 37/37 and final P0/P1/P2=0 review are green; the first latest-source E2E red was a stale fixture wording assertion and remains recorded. The production copy transaction created fresh author copy 13 from the unchanged source; its cutoff is `2026-08-02T06:33:47.698Z`, with inherited History=3 and metrics=23 but post-cutoff counts both 0. Run only suggestion → process → inline Diff → accept → Safe Undo there. That single click is also the only MiniMax structured-schema canary; do not add a separate paid attempt.
