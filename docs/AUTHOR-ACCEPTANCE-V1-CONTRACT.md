...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. Project card and Inline are closed. After the fifth-copy content-boundary and sixth-copy hardcoded-ID diagnostics, exact candidate `2073406` completed a real seventh-copy Chapter proposal and author acceptance. Operation `2acf8b66…` produced one applied History on the first chapter; disk exactly matches after revision `b0462641…`, while the unchanged source remains `9d089886…`. The write journey passed, but the live UI retained stale “selected / not yet written” hunk cards alongside the authoritative “safely applied” terminal. The Renderer repair replaces all pre-commit decisions with one completion card after a trusted no-residual commit; UX 9/9, Renderer dynamic 30/30, full test, approved-GUI verify, and forced real Electron 37/37 pass. Restart verification of the already accepted chapter is required, but the author must not repeat generation or the content decision. Plan, Research, the authorized image journey, and Graph/recovery remain open. None of this constitutes complete V0 acceptance or publication authorization; current totals remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
