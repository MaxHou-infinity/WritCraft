...
> Status: **frozen contract; `writ-craft@0.1.1` Developer Preview is public, while real-author acceptance remains open and is the sole `RM-1.0 / 0.1.2` target**. The fourth-copy project card and fifth-copy Inline accept/reject/Safe Undo subjourneys are closed and must not be repeated. The fifth-copy `b5.content` failure was zero-write diagnostic evidence and led to bounded content retry. The sixth-copy rerun exposed a separate production prompt contradiction: the exact example hardcoded `blockId:"b1"` while Main required dynamic `b2`; operation `814b0f1e…` failed after 24.83 seconds with no ChangeSet, History, or Markdown change. Exact candidate `2073406` now derives the example from the current Main block and keeps strict blockId validation unchanged. Chapter 21/21, full test, approved-GUI verify, and forced real Electron 37/37 pass, including non-default ID prompt canaries. The candidate is pushed and the seventh fresh eligible copy is open in the source App; the author now reruns only Chapter/Plan. The unchanged source remains `9d089886…`; Research, the authorized image journey, and Graph/recovery remain open. None of the release, synthetic, preparation, failed-profile, or invalidated-copy evidence constitutes complete V0 acceptance. Current totals and restart order remain authoritative only in `v0/DEVELOPMENT-STATUS.md`.
