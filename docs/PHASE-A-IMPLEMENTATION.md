...
> **当前实现停点**：`writ-craft@0.1.1` 已从 candidate `c65981e` 发布到 npm `preview`，同一 exact tag 的 GitHub `v0.1.1` prerelease 已创建，`latest` 仍为 `0.1.0`。当前唯一目标是 `RM-1.0 / 0.1.2` 真实作者闭环：项目卡、Inline 与 Chapter 已完成；第七副本的第六次 Plan 在 107,623 ms 后仍于里程碑 7任务 1 返回非数组 `targetPaths`，因此 `94e6099` 的 prompt/retry 方案不再视为关闭。0.0BU 将 Plan authority 迁移到单一 `submit_project_plan` tool input + 完整 schema + named tool choice，Main 保留全部 strict/零本地修复语义门禁。定向、完整 test、批准 GUI verify 和真实 Electron 37/37 通过，独立复审 P0/P1/P2=0；仍待真实 MiniMax named-tool canary 与后续 Research/Image/Graph 旅程。0.1.2 尚未发布，最终签字和发布授权均未取得；精确红绿与当前门禁只看 `v0/DEVELOPMENT-STATUS.md`。
