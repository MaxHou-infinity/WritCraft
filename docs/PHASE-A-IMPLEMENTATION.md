...
- **Chapter 生成**：真实作者的 `b5.content` 红灯证明 strict envelope 通过不等于每个内容区块可用。只允许把 CRLF/外层换行规范化为传输格式；strict JSON/schema/block id 不修补、不重试。内容为空、NUL、字符或字节越界时，全章最多原位重生当前区块一次；retry 前必须重验 `edit.md`、目标与上下文 revision，不得携带失败模型正文。第二次失败零写入终止，不得拼出部分 ChangeSet。
