...
> **当前实现停点**：`writ-craft@0.1.1` 已从 candidate `c65981e` 发布到 npm `preview`，同一 exact tag 的 GitHub `v0.1.1` prerelease 已创建，`latest` 仍为 `0.1.0`。当前唯一目标是 `RM-1.0 / 0.1.2` 真实作者闭环：项目卡、Inline 与 Chapter 已完成；第七副本的第七次 Plan operation `706fa3e5529d4cda9cf0e2c3ecfe211c` 已证明 named tool 请求获 provider 接受，但在 54,660 ms 后命中 `max_tokens`，且保持零写入。0.0BW 保留单一 `submit_project_plan` tool input 与全部 strict Main 门禁，把原约 419 KiB 合法生成空间收敛到单次两里程碑×每里程碑两任务、双目标和短字段；拒绝 escape amplification 并按 Unicode code point 对齐后，填满全部唯一 Emoji 字段与 80 字符路径的 fixture 为 6,120 bytes。不扩大 token/timeout 或自动付费重试。默认写作模型已经是 MiniMax-M3；公开中国区文档尚未确认 M3/1M 上下文合同，因此现有显式上下文安全门禁不放宽。Plan 25/25、相关定向套件、完整 test/verify、强制真实 Electron 37/37 均通过，独立终审 P0/P1/P2=0。生产修复已提交为 exact candidate `c6d8c44` 并重启开发 App；仍待一次真实 provider canary 与后续 Research/Image/Graph 旅程。0.1.2 尚未发布，最终签字和发布授权均未取得；精确红绿与当前门禁只看 `v0/DEVELOPMENT-STATUS.md`。
...
- **Plan 生成**：`docs/PLAN-STRICT-V1-CONTRACT.md` 已从文本 JSON 更新为工具协议。真实作者在里程碑 7任务 1再次输出非数组证明：自由生成大型重复 JSON 时，prompt、示例和一次重试不是类型约束。生产必须定义唯一 `submit_project_plan` 工具，以完整 `input_schema` 重复声明顶层/里程碑/任务的 required、array bounds 与 `additionalProperties:false`，并使用 named tool choice；Main 只接受 `stop_reason=tool_use` 且原始响应恰有一个同名、合法、plain-JSON 工具 input。thinking/text 不得成为 authority，旧 `end_turn` 文本 JSON、错误/多个/畸形 tool call 均失败关闭，绝不本地截取、剥围栏、猜数组或静默回退。结构数组错误仍至多共享一次重试预算；重试不得携带失败 input，必须在第二次调用前重验文件树、`edit.md` 与全部显式上下文，第二次失败即终止。真实 Electron fixture 必须断言 forced tool/schema，使用 thinking + text + tool_use，并在最后一个允许里程碑注入非数组 `targetPaths`；Plan 定向门禁统一使用 `npm run verify:plan`。官方示例只展示 `tool_choice:auto`，所以 named forced 形态还必须通过一次真实 MiniMax canary，自动化不得冒充 provider 签字。
