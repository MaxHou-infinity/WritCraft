...
- **Plan 生成**：`docs/PLAN-STRICT-V1-CONTRACT.md` 已实现；request/project/target revision ownership、strict JSON、stopReason、单文本块、失败终态、资源上限及 identifier-only Plan→Changes provenance 均已冻结并验证。真实外围文本失败证明 prompt 必须逐字声明与 scanner 一致的首尾字符、禁围栏、禁外围说明、禁第二对象和 exact-key 边界；不得用本地截取或剥围栏“修复”模型内容。只允许对 scanner 精确分类的外围文本做一次同操作格式重试；重试不得携带失败输出，且必须在第二次调用前重验文件树、`edit.md` 与全部显式上下文。第二次失败即终止。真实 Electron 必须让 fixture 首次返回 fenced/peripheral JSON，并证明同一次点击恢复且 Markdown/History 零写入。
