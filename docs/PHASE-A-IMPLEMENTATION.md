...
> **当前实现停点**：`writ-craft@0.1.1` 已从 candidate `c65981e` 发布到 npm `preview`，同一 exact tag 的 GitHub `v0.1.1` prerelease 已创建，`latest` 仍为 `0.1.0`。当前唯一目标是 `RM-1.0 / 0.1.2` 真实作者闭环：项目卡、Inline 与 Chapter 已完成；第七副本的 Plan 先暴露外围文本、再暴露第二任务 `targetPaths` 非数组 P1。exact production candidate `94e6099` 保持 strict parser/零本地修复，以一次共享、依赖重验的结构重试关闭两类缺口，当前定向 22/22、完整 test/批准 GUI verify、真实 Electron 37/37 通过；仍待独立复审、同一真实目标单次复验与后续 Research/Image/Graph 旅程。0.1.2 尚未发布，最终签字和发布授权均未取得；精确红绿与当前门禁只看 `v0/DEVELOPMENT-STATUS.md`。
...
- **Plan 生成**：`docs/PLAN-STRICT-V1-CONTRACT.md` 已实现；request/project/target revision ownership、strict JSON、stopReason、单文本块、失败终态、资源上限及 identifier-only Plan→Changes provenance 均已冻结并验证。真实失败证明 prompt 必须逐字声明与 scanner 一致的首尾字符、禁围栏、禁外围说明、禁第二对象和 exact-key 边界，同时明确每个里程碑/任务都重复完整结构、每个列表字段始终为 JSON 数组；不得本地截取、剥围栏或把字符串猜成路径数组。精确外围文本与数组形状错误只共享一次同操作重试预算；重试不得携带失败输出，必须在第二次调用前重验文件树、`edit.md` 与全部显式上下文，第二次失败即终止且绝不发起第三次调用。真实 Electron 必须让 fixture 的第二个任务返回非数组 `targetPaths`，并证明同一次点击恢复且 Markdown/History 零写入。Plan 定向门禁统一使用 `npm run verify:plan`，不得凭记忆拼测试文件名。
