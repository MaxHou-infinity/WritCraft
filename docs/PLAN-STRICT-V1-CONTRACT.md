...
> Status: the strict-text protocol was superseded by the 0.0BU single-tool boundary. Real operation `706fa3e5529d4cda9cf0e2c3ecfe211c` then proved MiniMax admitted that named-tool request but exhausted the local 8192-token generation budget after 54,660 ms. The old legal schema envelope could reach about 419 KiB. 0.0BW closes one planning iteration structurally at two milestones, two tasks per milestone, two targets per task, two dependencies and short bounded fields. Schema and Main both reject C0 controls and unpaired UTF-16 surrogates, preventing JSON escape amplification while retaining valid pairs such as emoji; Main and Renderer count Unicode code points to match JSON Schema. A fixture filling every count, ID, every unique 80-character path and every unique list/narrative field with four-byte emoji is exactly 6,120 bytes, below 6 KiB and leaving more than 2 KiB of byte headroom for the tool response envelope. The 80-character path bound covers every current author-acceptance Markdown path. The 8192 cap, 90-second deadline, zero automatic retry for `max_tokens`, strict Main validation and zero local coercion remain unchanged. MiniMax adapter 17/17, Plan 25/25, handoff 15/15 + 5/5, UI 16/16, Assistant 11/11, full test/verify and forced Electron 37/37 pass; independent review is P0/P1/P2=0. One real provider canary remains required. Authoritative current status remains `v0/DEVELOPMENT-STATUS.md`.
