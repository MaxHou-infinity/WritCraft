...
> Acceptance checkpoint (2026-08-03): exact candidate `e8b9588` completed the real-author journey in author copy 12. That copy had an exact pre-run zero-write baseline after its earlier red attempts: 14/14 Markdown files matched the selected source and post-cutoff History was 0. The new provider call returned the valid structured changes branch and displayed the expected inline red-delete/green-add Diff. The author accepted one chapter change at `2026-08-03T05:24:07.306Z` and completed Safe Undo at `2026-08-03T05:27:10.888Z`; the exact History record is `undone`, all 14 Markdown files again match the source, and the recovery directory is empty. This bounded before/after evidence closes both the unified-task author journey and the MiniMax `oneOf`/`const` canary without another paid run. Fresh copy 13 remains unused and must not be used to repeat the signed journey. Publication is outside this contract and still requires separate owner authorization.
