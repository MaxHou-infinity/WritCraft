...
> 状态：**V0 候选原型；0.0X 可信根 fd 项目根链绑定已完成全链验证，真实作者/付费/正式发布门禁仍开放** · 发起方：Max
...
> 当前停点：0.0X 不再由 Main 直接 `open(rootPath)`；native helper 从可信 `/` fd 逐段 no-follow 绑定项目根外层祖先，并在每个 hash batch 前后重走完整根链。当前证据为 worker **15/15**、Watcher **31/31**、cross-layer **11/11**、Large **6/6**、package **8/8**、release **7/7**、全量 test/verify、Persistent **3/3**、真实 Electron **32/32**。0.0W 唯一 watcher P2 已关闭；0.0X 独立复审 **P0=0/P1=0/P2=2**，待补 async-close 与构造错误脱敏回归。
...
> Graph 性能复验没有用单次重试绿灯覆盖失败：索引、布局、baseline 所有权、元素重挂、scene detach 与 dispatch-only 计时缺口均已进入根因修复和回归。该历史性能专项门禁等待两帧并读取实际 CSS 可见状态与 SVG 布局；同一源码无中间修改连续两次全链均为 **26/26**。当前签字项目总链是 0.0X 的 32/32；精确证据只看状态台账。

- 项目 / 文件 / 段落三级工作区、`edit.md`、Inline Diff、localized ChangeSet/撤销、权威 Context、来源/脚注、recovery、Cursor 式标签和一致性图谱均已接入；当前源码已完成 Onboarding v2 自动化产品链。0.0X ad-hoc App/ZIP 已按当前源码重建并验证，但仍不是 Developer ID/公证/Gatekeeper 发布产物，禁止分发。
...
- 0.0X 当前源码已通过 `npm test`、Electron-enabled `npm run verify`、Persistent **3/3**、强制真实 Electron **32/32**、package **8/8** 与 release **7/7**。16 MiB `MAX_REQUEST_BYTES` 与可信根 fd 外祖先绑定均已关闭；文件选择器之前、`readdir/fs.watch` 非原子性、同 UID 持有 fd 写入及 0.0U reserve residual 是明确范围边界，不能写成全路径/全并发原子安全。
