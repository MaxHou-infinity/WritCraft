...
第一次使用时，先打开 `edit.md`，把“为什么写、写给谁、要讲清什么”告诉笔触。你可以直接进入章节，通过项目对话、段落改写和一致性图谱推进作品。当前公开的 `0.1.2` Preview 已接入空项目“结构规划”、已有稿件“写作导航”，以及“处理建议 → 正文内 Diff → 接受或撤销”的统一任务流。
...
`0.1.2` 已以“结构规划 / 写作导航”替代旧 Plan 任务图，并完成“处理这个建议 → 同界面正文 Diff → 接受/拒绝/撤销”的统一任务流、图片受影响路径与真实作者验收；现已公开到 npm `preview` 与 GitHub prerelease。

## Preview 边界

- 目前仅支持 macOS arm64 / x64，通过终端启动。
- 尚未提供 Apple Developer ID 签名与公证的 `.app` 安装包。
- 当前公开 Preview 为 `writ-craft@0.1.2`；安装时请始终显式使用 `@preview`。`latest` 仍保留为上一稳定指针 `0.1.0`，不代表本次 prerelease。
