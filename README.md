...
> 状态：**V0 候选原型；0.0Z 超长 `edit.md` 分区上下文与 Inspector 披露已实现，真实作者/付费/正式发布门禁仍开放** · 发起方：Max
...
> 当前停点：0.0Z 让 Chat/Context 在 `edit.md` 超过 6000 字符或 18 KiB 时，按 Markdown 标题整章编译，优先保留项目主旨、范围、关键实体和时间/关系约束；右侧 Inspector 会逐章显示已使用/已省略、原因、大小并以 revision 门禁跳回原文。当前证据为 Context Resolver **26/26**、Inspector **8/8**、Chat **11/11**、Policy **10/10**、package **8/8**、release **7/7**、全量 test/verify、Persistent **3/3**、最终源码真实 Electron 连续两次 **33/33**，独立复审 **P0=0/P1=0/P2=0**。下一本地批是 Chat 最近对话摘要/多轮连续性，之后才是普通 Markdown 回收区恢复 UI。
...
> Graph 性能复验没有用单次重试绿灯覆盖失败：索引、布局、baseline 所有权、元素重挂、scene detach 与 dispatch-only 计时缺口均已进入根因修复和回归。该历史性能专项门禁等待两帧并读取实际 CSS 可见状态与 SVG 布局；同一源码无中间修改连续两次全链均为 **26/26**。当前签字项目总链是 0.0Z 的 33/33；精确证据只看状态台账。

- 项目 / 文件 / 段落三级工作区、`edit.md` Onboarding/落盘与分区 Prompt、Inline Diff、localized ChangeSet/撤销、单轮权威 Context、来源/脚注、草稿/冲突恢复、Cursor 式标签和一致性图谱均已接入；当前源码已完成 Onboarding v2 自动化产品链。这里的“恢复”不包含普通 Markdown 回收区的应用内列表/恢复入口。0.0Z ad-hoc App/ZIP 已按当前源码重建并验证，但仍不是 Developer ID/公证/Gatekeeper 发布产物，禁止分发。
...
- 0.0Z 当前源码已通过 `npm test`、非沙箱 `npm run verify`、Persistent **3/3**、强制真实 Electron 连续两次 **33/33**、package **8/8** 与 release **7/7**。超长 Prompt 分区能力当前只接入 Chat/Context；Chapter、Inline Rewrite 等模式仍保持各自既有 Prompt 合同。
