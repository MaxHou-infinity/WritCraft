...
第一次使用时，先打开 `edit.md`，把“为什么写、写给谁、要讲清什么”告诉笔触。你可以直接进入章节，通过项目对话、段落改写和一致性图谱推进作品。候选版本 `0.1.2` 已接入空项目“结构规划”、已有稿件“写作导航”，以及“处理建议 → 正文内 Diff → 接受或撤销”的统一任务流；当前公开的 `0.1.1` Preview 尚不包含它们。
...
`0.1.2` 已以“结构规划 / 写作导航”替代旧 Plan 任务图，并完成“处理这个建议 → 同界面正文 Diff → 接受/拒绝/撤销”的统一任务流及真实作者最短链验收。它仍是未发布候选：真实 GUI 图片评分/终态、精确候选发布复验和 npm 身份恢复尚未关闭，完成前不会发布 0.1.2。
