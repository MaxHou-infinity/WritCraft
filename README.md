...
第一次使用时，先打开 `edit.md`，把“为什么写、写给谁、要讲清什么”告诉笔触。你可以直接进入章节，通过项目对话、段落改写和一致性图谱推进作品。开发中的 `0.1.2` 已接入空项目“结构规划”和已有稿件“写作导航”；当前公开的 `0.1.1` Preview 尚不包含它们。
...
| 日常写作 | 项目/文件/选区对话、段落改写、Chapter，以及正在开发的建议→正文内 Diff 统一任务 |
...
`0.1.2` 已以“结构规划 / 写作导航”替代旧 Plan 任务图；真实作者验收又暴露多页面 AI 修改流程过重和等待无终态。当前正在实现“处理这个建议 → 同界面正文 Diff → 接受/拒绝/撤销”的统一任务流，完成前不会发布 0.1.2。
