  ...
  try {
    ...
    try {
      ...
      const diagnostics = await instance.client.evaluate(`(() => ({
        ...
        bodyTail: document.body.textContent.slice(-1800),
      }))()`).catch(() => null);
    ...
    await new Promise(resolve => setTimeout(resolve, 18_500));
    ...
    try {
      ...
      const diagnostics = await instance.client.evaluate(`(() => ({
        ...
        progress: document.getElementById('ai-task-progress')?.outerHTML || '',
        bodyTail: document.body.textContent.slice(-2200),
      }))()`).catch(() => null);
    ...
    try {
      ...
      const diagnostics = await instance.client.evaluate(`(() => ({
        ...
        status: document.getElementById('changes-status')?.textContent || '',
        preview: document.getElementById('changes-preview')?.textContent || '',
        issues: [...document.querySelectorAll('.issue-card')].map(card => card.textContent.slice(0, 300)),
        bodyTail: document.body.textContent.slice(-2200),
      }))()`).catch(() => null);
      error.message += `; diagnostics=${JSON.stringify(diagnostics)}; log=${JSON.stringify(instance.logRef.value.slice(-6000))}`;
      throw error;
    ...
    assert.deepStrictEqual(snapshotMarkdownFiles(copyRoot), copyBefore,
      'all preview/undo/trashed-image paths must restore the derived copy');
