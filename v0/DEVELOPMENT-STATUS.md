...
> 当前源码证据：**旧的未发布 0.1.2 candidates `45b1815`、`9a05c44`、`89658f0` 与 `6ed6f89` 均已被后续生产修复失效，不得发布或复用受影响验收证据。当前 exact production candidate 为 `30a06b0 fix(history): bind undo progress ownership`：Safe Undo 的阶段专属长等待文案、progress/busy/undo owner token、跨项目 fail-closed 清理，以及 NO_KEY 设置往返、A→B 隔离、成功清理和迟到响应动态证据均已合入。第二轮独立复审为 **P0=0/P1=0/P2=1**，唯一 P2 是 candidate 提交内文档仍写“未提交/待复审”，本次 docs-only 收口即修正该偏差，不改变生产候选。定向 Changes UX 8/8、Renderer dynamic 29/29、Onboarding Workspace dynamic 4/4，完整 `npm test`、批准 GUI 环境 `npm run verify`、强制真实 Electron 37/37、npm Preview 10/10、本地 installed 2/2、Node 22/npm 10 arm64 2/2、Node 24/npm 11 x64 2/2 均通过；生产依赖 0 vulnerabilities。候选包为 120 files / 574,770 bytes / 2,631,216 unpacked / shasum `4d2805ca2894d8dcd5857cec414aab850a51ff28`。第三个 fresh 作者副本现在可以从未变验收源创建。npm/GitHub 未发布 0.1.2。**
