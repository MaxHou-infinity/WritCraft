...
> 当前源码证据：**旧的未发布 0.1.2 candidates `45b1815`、`9a05c44` 与 intermediate `89658f0` 均已被后续生产修复失效，不得发布或复用受影响验收证据。当前 exact candidate 是 `6ed6f89 fix(onboarding): route missing key to settings`：已同时关闭 Safe Undo 假 AI 长等待和项目卡 `NO_KEY` 通用重试 P1；`NO_KEY` 明示未调用 AI/未改文件，提供“打开设置”，并按 project instance 保留项目卡草稿，项目切换或成功完成会清除草稿。定向项目卡 UI 12/12、Renderer 27/27、Main/Preload 14/14、Workspace 22/22，完整 `npm test`、批准 GUI 环境 `npm run verify`、强制真实 Electron 37/37、npm Preview 10/10、本地 installed 2/2、Node 22/npm 10 arm64 2/2、Node 24/npm 11 x64 2/2 均通过；生产依赖 0 vulnerabilities。包为 120 files / 574,171 bytes / 2,628,827 unpacked / shasum `613a963b9ab2b18c746341448bcb9b4c3f9aab1e`。仍须独立复审和第三个 fresh 作者副本。npm/GitHub 未发布 0.1.2。公开 0.1.1 的 registry 证据保持不变。**
