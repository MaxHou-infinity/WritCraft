...
> 最后更新：2026-07-30（Asia/Shanghai，0.0BC · 独立复审发现并修复 Safe Undo 长等待/跨项目 owner 竞态）
...
> 当前源码证据：**旧的未发布 0.1.2 candidates `45b1815`、`9a05c44`、`89658f0` 与 `6ed6f89` 均已被后续生产修复失效，不得发布或复用受影响验收证据。对 `6ed6f89` 的新独立复审为 P0=0/P1=2/P2=1：Safe Undo 超过 10 秒仍被公共计时器改写为 AI 文案，且旧项目撤销的迟到 `finally` 可清理新项目任务；NO_KEY 的 Workspace 行为证据不足。当前工作树已加入阶段专属长等待文案、progress/busy/undo owner token、跨项目 fail-closed 清理，以及 NO_KEY 设置往返、A→B 隔离、成功清理和迟到响应动态测试。定向 Changes UX 8/8、Renderer dynamic 29/29、Onboarding Workspace dynamic 4/4，完整 `npm test`、批准 GUI 环境 `npm run verify`、强制真实 Electron 37/37、npm Preview 10/10、本地 installed 2/2、Node 22/npm 10 arm64 2/2、Node 24/npm 11 x64 2/2 均通过；生产依赖 0 vulnerabilities。当前未提交包为 120 files / 574,770 bytes / 2,631,216 unpacked / shasum `4d2805ca2894d8dcd5857cec414aab850a51ff28`。必须先形成新的 exact candidate 并重新独立复审，才允许创建第三个 fresh 作者副本。npm/GitHub 未发布 0.1.2。**
> 当前作者验收进度：**第二个 fresh 副本已完成项目卡、Inline 拒绝、Inline 接受与精确 Safe Undo，磁盘/History 事务通过：最新正文记录变为 `undone`，正文恢复 `1bdb3a5…`，`edit.md` 保持 `e9bacec…`。但该副本先后暴露撤销后假 AI 计时、长等待文案退化和跨项目 owner 竞态；因此只保留为功能/红灯证据，不能作为最终发布签字。第三个 fresh 副本尚未创建；必须等待新 candidate 的第二轮独立复审 P0/P1 为零。**
...
- **门禁**：`6ed6f89` 已失效。先提交当前源码/测试/文档形成新 exact candidate，再对该提交做新的独立复审；在 P0/P1 为零前不得创建第三个 fresh 副本。
...
- [ ] 只修复验收中发现的 P0/P1 与决定阻断发版的 P2；不得插入 `0.2.0+` 功能。
...
- [x] Inline Rewrite 自动化链：`end_turn`/严格 JSON、输出边界、保留路径禁写、Main capability/ACK、durable reconciliation、全局 mutation guard、接受时依赖复核、History/undo 与原位红绿 Diff 已完成；独立复审 P0/P1=0，**21/21** 是历史专项 Electron 证据，0.0AB 当时总链为 **35/35**，当前见顶部当前里程碑。
