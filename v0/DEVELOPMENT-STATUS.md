...
> 当前源码证据：**旧的未发布 0.1.2 candidates `45b1815`、`9a05c44`、`89658f0`、`6ed6f89` 与 `30a06b0` 均已被后续生产修复失效，不得发布或复用受影响验收证据。当前生产树修复了“已提交 edit.md 后跳过初始文件仍显示结果未变”的陈旧终态文案，并新增跨真实 Changes 回调的动态回归；Onboarding Renderer dynamic 从 29/29 提升到 **30/30**，Changes UX 8/8。完整 `npm test` 前两次均在与本修复无关的 native Markdown trash helper 启动阶段以 `bind timed out` 红灯（各 19/20，具体用例不同），单独同测试 20/20；根因是测试把 22 个 helper 的启动/恢复预算硬编码为 1 秒，而生产默认 30 秒。测试启动预算现独立为 5 秒，`ready()` 后的故障注入仍为 100ms；聚焦 20/20，随后完整 `npm test` exit 0。新的 exact candidate 尚待提交、复审和全门禁验证；npm/GitHub 未发布 0.1.2。**
...
- **保留红灯与测试根因**：首次和第二次完整 `npm test` 都在 posttest 的 native Markdown trash helper 以 `bind timed out` 红灯、各 19/20，但失败用例不同；聚焦复跑 20/20。测试曾把 helper bind/recovery 硬编码为 1 秒，完整套件连续 clang 编译/启动会跨过该偶然窗口。现以命名常量将测试启动预算改为 5 秒，生产默认仍为 30 秒，`ready()` 后的 crash/unknown 注入继续设置 100ms。修正后聚焦 20/20，完整 `npm test` exit 0。
- **证据失效与重启顺序**：这是生产 Renderer 变更，故 `30a06b0` 和第三副本项目卡证据失效。必须形成新 exact candidate → 独立复审 P0/P1 为零 → 完整 verify/真实 Electron/安装矩阵 → 创建第四 fresh 副本，五段旅程从项目卡重跑。
